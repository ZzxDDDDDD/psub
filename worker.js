...
var src_default = {
    async fetch(request, env) {
        ...
        const frontendUrl = 'https://raw.githubusercontent.com/ZzxDDDDDD/psub/main/frontend.html';
        ...
        url.search = url.search.replace(/%253D/g, '%3D');
        ...
        } else if (rpResponse.status === 400) {
            // 创建 rpResponse 的副本
            const modifiedResponse = rpResponse.clone();

            // 提取 modifiedRequest 的相关信息
            const modifiedRequestInfo = `Modified Request:
          URL: ${modifiedRequest.url}
          newURL: ${urlString}
          Method: ${modifiedRequest.method}
          Headers: ${JSON.stringify(Object.fromEntries(modifiedRequest.headers), null, 2)}`;

            // 获取 rpResponse 的原始内容
            const originalResponse = await modifiedResponse.text();

            // 修改响应内容
            const modifiedBody = `Bad Request: ${modifiedResponse.statusText}
...
function replaceSS(link, replacements, isRecovery) {
    ...
    const [linkPart, queryPart] = replacedString.split(/\?|%3F/);
    if (queryPart) {
        const encodedQueryPart = queryPart.split('&').map(param => {
            const [key, ...values] = param.split('=');
            const encodedValue = values.join('=').replace(/=/g, '%3D');
            return `${key}%3D${encodedValue}`;
        }).join('&');
        replacedString = `${linkPart}?${encodedQueryPart}`;
    }
...
function replaceTrojan(link, replacements, isRecovery) {
    ...
    let replacedLink;
    if (isRecovery) {
        replacedLink = link.replace(regex, (match) => cReplace(match, uuid, replacements[uuid], server, replacements[server]));
    } else {
        replacedLink = link.replace(regex, (match) => cReplace(match, uuid, randomUUID, server, randomDomain));
    }

    const beforeInsert = replacedLink.replace(/=/g, '%3D');
